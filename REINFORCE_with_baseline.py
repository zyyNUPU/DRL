...
#---------------------------------------------------------------
#REINFORCE_with_baseline是A2C的一种特例（all step类型，A2C是one step或者muti step类型），他是基于一条真实完整的路径
#---------------------------------------------------------------
EPS = 1e-12

SavedAction = namedtuple('SavedAction', ['log_prob', 'value'])
# # 创建实例
# action = SavedAction(log_prob=0.5, value=10)
...
class Policy(nn.Module):
    ...
    def forward(self, state):
        ...
        action_prob = F.softmax(out, dim=-1)#policy network 的输出

        for layer in self.v_layers:
            value = layer(x)
        state_value = self.v_output_layer(value)# Q值-特定的实数
    ...
    def select_action(self, state):
        ...
        action = dist.sample() # choose action from the distribution，返回的结果是单个值的张量
        
        self.saved_actions.append(SavedAction(dist.log_prob(action), state_value)) # save to action buffer

        return action.item()#将张量中的单个值拿出来，转换成一个标准的python数据类型
    ...
    def calculate_loss(self, gamma=0.999):
        ...
        for t in range(len(rewards)-1, -1, -1): # calculate disounted returns in each time step，第一个-1是循环的结束值，第二个-1是步长
            disc_returns = (returns[0] if len(returns)> 0 else 0)
            G_t = gamma * disc_returns + rewards[t]
            returns.insert(0, G_t) # insert in the beginning of the list Ut列表
            state_value = saved_actions[t][1]#获取V值
            state_value_list.append(state_value)# V列表
        ...
        for step in range(len(saved_actions)):
            ...
            adv = adv_list[step]
            policy_losses.append(adv * log_prob)# 策略损失

        value_loss = F.mse_loss(torch.tensor(state_value_list), torch.tensor(returns))#这里的价值损失是MSE
        policy_loss = torch.stack(policy_losses, dim=0).sum()#这个前面的操作是将列表堆叠成一维的张量再转换成一个一维张量。
...
if __name__ == '__main__':
    ...
    env = gym.make(args.env)#if __name__ == '__main__':没有被封装在任何函数内，所以是全局变量
