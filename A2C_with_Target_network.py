...
class A2C:
    ...
    def compute_policy_loss(self, bs, blogp_a, br, bd, bns):
        ...
        # 计算policy loss。
        #用 with torch.no_grad() 包裹，这意味着它不会追踪梯度。
        #因此，在这种情况下，advantage 的计算不会影响到 self.V 的计算图。
        with torch.no_grad():
...
def train(args, env, agent: A2C):
    ...
    for step in range(args.max_steps):
        ...
        if done is True:
            ...
            value_loss.backward(retain_graph=True)#这里保留了计算图，不选择保留，就直接会删除。
            V_optimizer.step()

            policy_loss = agent.compute_policy_loss(bs, blogp_a, br, bd, bns)
            pi_optimizer.zero_grad()
            policy_loss.backward()# 在这里可以不保留计算图，因为这是最后一次 backward 调用
            pi_optimizer.step()
